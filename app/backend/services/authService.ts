...
async function issueConfirmCode(userId: string, email: string, name: string)
{
    ...
    await sendEmail('confirm-registration', { to: email, subject: `${code} is your Maturin confirmation code`, html: confirmCodeEmailHtml({ name, code }) });
}
...
export async function confirmRegistration(email: string, code: string)
{
    ...
    await sendEmail('welcome', { to: updatedUser.email, subject: 'Welcome to Maturin', html: welcomeEmailHtml({ name: updatedUser.name }) });
...
export async function changePassword(userId: string, currentPassword: string, newPassword: string)
{
    ...
    await sendEmail('password-changed', { to: user.email, subject: 'Your Maturin password was changed', html: passwordChangedEmailHtml({ name: user.name }) });
}
...
export async function deleteAccount(userId: string, password: string)
{
    ...
    await sendEmail('account-deleted', { to: user.email, subject: 'Your Maturin account has been deleted', html: accountDeletedEmailHtml({ name: user.name }) });
...
export async function requestPasswordReset(email: string)
{
    ...
    await sendEmail('reset-password', { to: user.email, subject: 'Reset your Maturin password', html: resetPasswordEmailHtml({ name: user.name, resetUrl }) });
}
...
export async function sendVerificationEmail(userId: string)
{
    ...
    await sendEmail('verify-email', { to: user.email, subject: 'Verify your Maturin email', html: verifyEmailHtml({ name: user.name, verifyUrl }) });
}
...
function resetPasswordEmailHtml({ name, resetUrl }: { name: string; resetUrl: string })
{
    return emailShell(`
      ...
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 20px;">
        We received a request to reset your Maturin password. This link expires in 1 hour.
      </p>
      <a href="${resetUrl}"
...
function confirmCodeEmailHtml({ name, code }: { name: string; code: string })
{
    ...
    return emailShell(`
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 6px;">Hi ${name},</p>
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 20px;">
        Enter this code to confirm your email and finish creating your Maturin account. It expires in 15 minutes.
      </p>
      ...
      <p style="color:#a8a29e;font-size:12px;margin:0;">
        If you didn't create a Maturin account, you can safely ignore this email.
      </p>
    `);
}
...
function welcomeEmailHtml({ name }: { name: string })
{
    return emailShell(`
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 6px;">Hi ${name},</p>
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 20px;">
        Welcome to Maturin — glad to have you. Small actions, done daily, add up to big change.
      </p>
      <a href="${FRONTEND_URL}"
        style="display:inline-block;background:#C4601A;color:#fff;text-decoration:none;padding:12px 28px;border-radius:10px;font-size:14px;font-weight:600;">
        Open Maturin →
...
function passwordChangedEmailHtml({ name }: { name: string })
{
    return emailShell(`
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 6px;">Hi ${name},</p>
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 20px;">
        Your Maturin password was just changed. If this was you, no action is needed.
      </p>
...
function accountDeletedEmailHtml({ name }: { name: string })
{
    return emailShell(`
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 6px;">Hi ${name},</p>
      <p style="color:#44403c;font-size:15px;line-height:1.6;margin:0 0 20px;">
        Your Maturin account and all associated data have been permanently deleted, as requested.
      </p>
      <p style="color:#78716c;font-size:13px;margin:0;">
